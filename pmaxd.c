...
// if (!xPL_parseCommonArgs(&argc, argv, FALSE)) exit(1);
  ...
  while ((c = getopt (argc, argv, "fvh")) != 255)
    switch (c)  {
      case 'f':
        foregroundOption = 1;
        break;        
...
//    sendBuffer(&PowerlinkCommand[Pmax_ARMAWAY]);
 ...
 //   sendBuffer(&PowerlinkCommand[Pmax_DISARM]);
  ...
  else if (strcmp(buffer,"StatusAlarm") == 0)
  {
    DEBUG(LOG_NOTICE,"Status");
...
//      n = write(newsockfd,"PowerMaxArmedAway",17);
    ...
    else if (gatestat.pmstatus == XplStatusPMArmingAway)
    {
      n = write(newsockfd,"PowerMaxArmingAway",18);
    }
    else
    {
  ...
  {
    DEBUG(LOG_NOTICE,"Unknown input!!!!......");
  }
}
...
void* serverThread(void *arg)
{
  ...
  while(1) {
    DEBUG(LOG_NOTICE,"Waiting on client......");
    ...
    MessageHandling(buffer, newsockfd);
    close(newsockfd);
  }
...
int main(int argc, char **argv) {
  ...
  if (!config_read_file(cf, "/home/pi/powermax/code/pmaxd.conf")) {
...
// 	commandBuffer.size=0;
 ...
 err = pthread_create(&(tid[1]), NULL, &serverThread, NULL);
 if (err != 0)
